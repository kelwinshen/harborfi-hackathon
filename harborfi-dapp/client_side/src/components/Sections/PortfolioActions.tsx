...
import { TabsAsset, TabsHistory } from ".";
...
const PortfolioActions = () => {
  ...
  useEffect(() => {
    if (!metamaskAccountAddress) {
      setTokenBalances({ [tokenA]: 0, [tokenB]: 0, [tokenC]: 0 });
      setNativeBalance(0);
    }
    ...
    const fetchPrices = async () => {
      try {
        const res = await fetch(
          "https://api.coingecko.com/api/v3/simple/price?ids=ethereum,usd&vs_currencies=usd,php,idr,thb"
        );
